...
class CodeValidator:
    ...
    def validate(self, code: str, hardware: str, user_message: str = "", domain: str = "embedded") -> Tuple[bool, List[Dict]]:
        """Check code against known rules"""
        issues = []
        
        # Universal: Check learned rules (from DB) could be added here
        
        # Domain Specific Validation
        if domain == "embedded":
            issues.extend(self._validate_embedded(code, hardware, user_message))
            
        return len([i for i in issues if i['severity'] == 'error']) == 0, issues

    def _validate_embedded(self, code: str, hardware: str, user_message: str) -> List[Dict]:
        """Hardcoded validation logic for Embedded/Robotics domain"""
        issues = []
