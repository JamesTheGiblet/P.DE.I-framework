...
class ShadowSuggestionEngine:
    ...
    def get_proactive_suggestion(self, user_input: str) -> Optional[str]:
        ...
        2. Query repo_index for user's most frequent companion modules
    ...
    def get_all_suggestions(self, user_input: str, generated_code: str) -> List[str]:
        ...
        if "L298N" in generated_code and "safety" not in generated_code.lower():
            suggestions.append("Drive system lacks safety timeout (Standard uses 5s failsafe). Add that?")
        ...
        for i, (msg_id, role, content) in enumerate(messages):
            if role == 'user' and i > 0:
                ...
                if prev_role == 'assistant':
                    # Did user correct the previous response?
...
class SmartLearner:
    ...
    def analyze_corrections(self, ai_interface=None):
        ...
        for row_id, original, corrected, reason in corrections:
            ...
            if corrected and original:
                ...
                # Strategy 3: Template Matching (Forge Theory) [Jan 2026 Roadmap]
                templates = self.match_templates(original, corrected, reason)
                if templates:
    ...
    def match_templates(self, original: str, corrected: str, reason: str) -> List[Dict]:
        """Match against known high-value methodology templates (Jan 2026 Roadmap)"""
        matches = []
        
        # Template 1: Forge Theory (Exponential Smoothing)
        # Pattern: current += (target - current) * k
        if "current" in corrected and "target" in corrected and "* k" in corrected:
            if re.search(r'\+=\s*\(\s*\w+\s*-\s*\w+\s*\)\s*\*\s*k', corrected):
                matches.append({
                    "rule": "Forge Theory: Use exponential smoothing (current += (target - current) * k)",
                    "find": r'\+=\s*k\s*\*\s*\(\s*\w+\s*[\+\-]\s*\d+\s*\)',
                    "replace": "current += (target - current) * k",
                    "confidence": 1.0,
                    "category": "forge_theory",
                    "priority": "CRITICAL"
                })
        return matches
